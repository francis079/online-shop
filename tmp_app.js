    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="card-footer">
          ...
          ${sellerUnlocked ? `<button class="button button-secondary" data-delete-id="${product.id}">Delete</button>` : ''}
        </div>
      </div>
    `;
...
async function addProduct(event) {
  ...
  try {
    ...
    if (apiMode === 'flask') {
      ...
      // PHP backend expects POST to api.php?resource=products
      response = await fetch('/api.php?resource=products', {
        method: 'POST',
        body: formData
      });
    }
...
async function deleteProduct(productId) {
  ...
  try {
    let response;
    if (apiMode === 'flask') {
      response = await fetch(`/api/products/${productId}`, { method: 'DELETE' });
    } else {
...
function unlockSeller() {
  if (selectors.sellerKeyInput.value === sellerKey) {
    sellerUnlocked = true;
